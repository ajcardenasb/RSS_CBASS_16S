...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncLen=c(220,190),maxN=0, maxEE=c(2,2), truncQ=10, rm.phix=TRUE, compress=TRUE, multithread=TRUE) 
head(out)
...
write.table(asv.final, "RSSvsCBASS_ASV_table_pooled.txt",  quote = FALSE)
write.table(track, "RSSvsCBASS_ASV_stats_pooled.txt", quote = FALSE)
