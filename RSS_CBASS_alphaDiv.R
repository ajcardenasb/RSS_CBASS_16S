...
asv_trn$Genotype=map$Genotype[match(rownames(asv_trn), rownames(map))]


## CB
asv.acu=subset(asv_trn, rownames(asv_trn) %like% "^CB")
adonis(asv.acu[,1:8660] ~ asv.acu$Temperature + asv.acu$Time +  asv.acu$Genotype)
...
adonis(asv.chr[,1:8660] ~ asv.chr$Temperature + asv.chr$Time + asv.chr$Genotype)
